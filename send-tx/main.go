...
// 发送交易：go run main.go --to <address> --amount <eth> - 发起 ETH 转账交易
func main() {
	toAddrHex := flag.String("to", "", "recipient address")
	amountEth := flag.Float64("amount", 0, "amount in ETH")
	flag.Parse()

	if *toAddrHex == "" || *amountEth <= 0 {
		log.Fatal("requires --to and --amount flags, and amount must be greater than 0")
	}
